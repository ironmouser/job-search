...
import { Copy, Loader2, ThumbsUp, RefreshCw, Minimize2, Maximize2, CheckCircle, ChevronDown, Edit2, Save, X, RotateCcw, Pencil, Send } from 'lucide-react';
...
export default function NetworkingAssetCard({
    ...
    const handleRegenerate = async (instruction: string) => {
        ...
        // Clear custom prompt UI on any regeneration
        ...
        try {
            ...
            if (!finalContent) {
                setError('The regenerated networking message appears to be empty. Please try again.');
    ...
    const savePreference = async () => {
        ...
        try {
            await fetch('/api/settings', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ networkingMessageTone: tone })
            });
            setSavedPref(true);
        } catch (err) {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            const res = await fetch(`/api/job/${jobId}/assets`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ networkingMessage: editContent }),
            });
            if (!res.ok) {
