...
export async function logAiCost(model: string, inputTokens: number, outputTokens: number, userId?: string) {
    ...
    } else if (model.includes('deepseek-v4.1-flash') || model.includes('deepseek-chat') || model.includes('deepseek-v3')) {
        costUsd = (inputTokens / 1_000_000) * 0.22 + (outputTokens / 1_000_000) * 0.66;
    } else if (model.includes('deepseek-v4.1-pro') || model.includes('deepseek-reasoner')) {
