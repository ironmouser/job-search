/**
 * worker/src/generic-agent/glm-navigator.ts
 *
 * GLMNavigator — Primary reasoning layer using GLM-5.3-Flash over the semantic AXTree snapshot.
 *
 * Responsibilities:
...
import { callGLMWorker, parseGLMJson } from '../ai/glm-client';
import { agentConfig } from '../config';
import { AgentDecision, AgentState, SemanticSnapshot } from './types';

export class GLMNavigator {
...
`;
  ...
  static async decideAction(
    ...
    if (!agentConfig.glmApiKey || !agentConfig.aiNavigationEnabled) {
      return {
        decision: {
          action: 'manual_intervention',
          confidence: 0,
          reason: 'GLM navigation disabled or API key missing',
        },
        promptTokens: 0,
        completionTokens: 0,
      };
...
Analyze the above snapshot and return the single best JSON action.`;
    ...
    try {
      const result = await callGLMWorker({
        messages: [
      ...
      const parsed = parseGLMJson<AgentDecision>(result.content);
      if (!parsed || !parsed.action) {
        return {
          decision: {
            action: 'stop',
            confidence: 0,
            reason: `[PARSE_FAILURE] Invalid JSON from GLM: ${result.content.slice(0, 100)}`,
          },
      ...
      return {
        decision: {
          action: 'stop',
          confidence: 0,
          reason: `GLM call failed: ${err.message}`,
        },
        promptTokens: 0,
        completionTokens: 0,
      };
