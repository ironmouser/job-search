...
const MP4_FILES = ['thumbs.mp4', 'lasso.mp4', 'head.mp4', 'fly.mp4'];

async function uploadMp4Videos() {
  ...
  for (const fileName of MP4_FILES) {
  ...
  console.log('All MP4 uploads completed!');
}

uploadMp4Videos().catch(err => {
  console.error('Error during MP4 upload process:', err);
  process.exit(1);
});
