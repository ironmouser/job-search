...
const HERO_FILES = ['hero.jpg', 'hero-dark.jpg'];
...
async function uploadHeroImages() {
  ...
  for (const fileName of HERO_FILES) {
  ...
  console.log('Hero image uploads completed!');
}

uploadHeroImages().catch(err => {
  console.error('Error during Hero upload process:', err);
  process.exit(1);
});
